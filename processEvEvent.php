...
$clubdb = array(array());
$DEBUGME = true;
$ignoreRe = '/(MTBO|TEST|RELAY|SCORE|STREET|BIKE|TEAM|DUO|NIGHT|MTB|SSS|SKI|MELBOURNE PAS|SCATTER|WALKER|SATURDAY O.*SERIES|SOS |URBAN|MAPRUN)/';
$logCount = 0;
$sprint = false;
...
function process_event($event_id, $race_id = 0)
    ...
    if (strlen($html) > 200)
        ...
        if (!parse_page($html, $skipped))
            if ($skipped)
                return true; // Force eventor event to be marked as processed
            else
                return false;
        ...
        {
        Trace($url.$event_id." contents was only ".strlen($html)." bytes ! Not processing.");
        return false;
        }
    }
...
function process_row(&$courseid, $item, &$count, &$prevtime)
    ...
    foreach ($items as $col)
        ...
        switch ($colclass)
            ...
            case "s" : 
                {
                $val = $col->textContent;
                // Note - score may be sent with no value in a line class in a mixed event
                if (!$gotScoreClass && $val > 0)
                    {
                    Trace("Dropping some results as they seem to be score event results, found score $val for $name");
                    $gotScoreClass = true;
                    return;  // Looks like a score event, skip rest of this class.
                    }
                }
            default: break;         
            }
    ...
    if (strlen($club) == 0 || strlen($name) == 0 || strlen($time) == 0)
        {
        Trace("Club, name or time not found, skipping result");
        return;
        }
...
function process_table(&$courseid, $item)
    ...
    if (strpos($item->getAttribute('class'), 'resultList') === false)
        {
        Trace("Didn't find resultList in table attributes, skipping");
        return;
        }
...
function parse_page(&$html, &$skipped) 
    {
    global $event;
    global $runnerid;
    global $ignoreRe;
