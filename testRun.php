...
//$url = "http://eventor.orienteering.asn.au/Events/ResultList?eventId=1363&eventRaceId=1413";
//$url = "http://eventoraustralia.orientering.se/Events/ResultList?eventId=1488&groupBy=EventClass";
//$url = "http://eventoraustralia.orientering.se/Events/ResultList?eventId=1793&eventRaceId=1859";
$url="https://eventor.orienteering.asn.au/Events/ResultList?eventId=5465&eventRaceId=5669&groupBy=EventClass";
$stageDate = "2018-03-30";
$stageName = "NOL Easter 2018 Prologue - Sandy Bay";
...
$ignoreRe = '/(MTBO|TEST|RELAY|SCORE|STREET|BIKE|TEAM|DUO|NIGHT|MTB|SSS|SKI|MELBOURNE PAS|SCATTER|WALKER|SATURDAY O.*SERIES)/';
$logCount = 0;
			...
			if ($maxDeviation > 0)
				...
				foreach ($row as $result)
					...
					if ($maxDeviation == $result->current_score - $score) 
						{
						Trace("Dropping ranker score: ".$score.", current_score: ".$result->current_score.", dev: ".$sd);
...
foreach ($results as $key => $result)
    ...
    $stats[$course]['ST'] != 0 && $zeros < 0 && $time > 0)
		{
...
function save_results()
	...
	$query .= "values ('".normalise_event_name($event['name'])."','".$event['date']."','".$event['url']."')";
	/*$result = $mysqli->query($query) or trigger_error($mysqli->error." ".$query);
	if (!$result)
			{
...
/*		$result = $mysqli->query ($query); 
		...
		$query = "insert into results (runnerid, eventid, points, class) values (";
...
function process_header(&$item)
  ...
  global $stageDate;
	global $stageName;
  $event['date'] = $stageDate;
	$event['name'] = trim(mysqli_real_escape_string($mysqli, $stageName));
